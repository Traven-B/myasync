module Hennepin
  HENN_BASE_URL_actual = "https://hclib.bibliocommons.com"
  HENN_BASE_URL_local = "http://localhost:3000/henn"

  HENN_BASE_URL = Local.local? ? HENN_BASE_URL_local : HENN_BASE_URL_actual
  ...
  def self.parse_checkedout_page(page)
    henn_checked_out_books_page = Nokogiri.parse(page)
    books_out = henn_checked_out_books_page.css("div.cp-checked-out-item").map do |book_part|
  ...
  def self.parse_on_hold_page(page)
    henn_on_hold_books_page = Nokogiri.parse(page)
    books_on_hold = henn_on_hold_books_page.css("div.cp-bib-list-item.cp-hold-item.ready_for_pickup").map do |book_part|
